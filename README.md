...
An application utilizing the OMDBApi with an index page with 4 pre-selected movies. The user can then click on the poster image which will route them to a detail page which will show additional info that movie. The user can also search for a movie or movies of their choice with which will update the table of movie(s).
